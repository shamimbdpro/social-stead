    ...
    <table id="table2" class="display table table-bordered table-striped table-hover">
      ...
      <?php for ($i = 0; $i < count($alldata); ++$i) { ?>
        ...
        <tr id="<?php echo $alldata[$i]->id; ?>">
          ...
          <td><?php echo $i+1; ?></td>  
          <td><?php echo $alldata[$i]->s_name; ?></td>

            <?php
                if($alldata[$i]->status==1){
                  ?>
                 <td> <a href="<?php echo base_url();?>publish/service/<?php echo $alldata[$i]->id;?>" class="green-text">Publish</a></td>
                  <?php
                }else{
                  ?>
                 <td> <a href="<?php echo base_url();?>unpublish/service/<?php echo $alldata[$i]->id;?>" class="red-text">Unpublish</a></td>
              <?php
                }
              ?>
          <td>    
            &nbsp;&nbsp;&nbsp;&nbsp;      
            <a class="ajaxDeleteD" id="<?php echo $alldata[$i]->id; ?>" href="<?php echo base_url(); ?>delete/service/<?php echo $alldata[$i]->id; ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-close bigico"></i></a>
            &nbsp;&nbsp;&nbsp;&nbsp;  
        </tr>
      <?php } ?>
      </tbody>
